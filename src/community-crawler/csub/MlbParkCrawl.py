...
class Crawler:
    ...
    def run(self, years, months, days):
        ...
        pool.close()
        sys.stdout.write(f"MLBPark 크롤링 완료\r")
        sys.stdout.flush()

    def crawlPage(self, page):
        url = f'http://mlbpark.donga.com/mp/b.php?p={page}&m=list&b=bullpen&query=&select=&user='
        time.sleep(0.02)
        res = req.get(url)
        res.encoding = None
        html = res.text
    ...
    def pageCalculation(self, date_Specified):  # 입력한 날짜의 글이 있는 페이지 까지 검색
        ...
        regen = 70  # 하루에 글이 평균적으로 작성되는 양(페이지 기준). 사이트마다 적당한 고정값을 줘야 검색이 빨라짐
        stop = True
        page_Target = True
        result = []
        ...
        while stop == True:
            url = f'http://mlbpark.donga.com/mp/b.php?p={((regen - 1) * 30) + 1}&m=list&b=bullpen&query=&select=&user='
            res = req.get(url)
            res.encoding = None
            html = res.text
        ...
        if stop == False:
            ...
            while stop == False:
                ...
                if stop == True:
                    ...
                    regen = regen - 1
        for i in range(1, (regen * 30) + 1, 30):
            result.append(i)
        return result