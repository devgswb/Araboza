...
class Crawler:
    ...
    def run(self, years, months, days):
        ...
        pool.close()
        sys.stdout.write(f"Clien 크롤링 완료\r")
        sys.stdout.flush()

    def crawlPage(self, page):
        url = f'https://www.clien.net/service/board/park?&od=T31&po={page}'
        time.sleep(0.02)
        res = req.get(url)
        html = res.text
        ...
        for i, w in enumerate(soup.select('div[class=list_title] > a[class=list_subject] > span[class=subject_fixed]')):
            w = w.get_text()
            w = w.split('\n')[0]
            # w = w.replace('\n', '')
            w = w.replace(',', ' ')
        ...
        for i, d in enumerate(soup.select('div.list_item.symph_row > div.list_time > span.time.popover > span.timestamp')):
            d = d.get_text()
            d = d.split(' ')[0]
            d = d.replace('\n', '')
        ...
        for t in range(len(write)):
            ...
            if datetime.datetime.strptime(self.day, "%Y-%m-%d") < self.endTime:
                return
            name = 'Clien'
            fpath = f'data/2/[{day[t].replace(".", "-")}]{name}.csv'
    ...
    def pageCalculation(self, date_Specified):  # 입력한 날짜의 글이 있는 페이지 까지 검색
        ...
        regen = 80  # 하루에 글이 평균적으로 작성되는 양(페이지 기준). 사이트마다 적당한 고정값을 줘야 검색이 빨라짐
        stop = True
        page_Target = True
        result = []
        ...
        while stop == True:
            url = f'https://www.clien.net/service/board/park?&od=T31&po={regen}'
            res = req.get(url)
            html = res.text
