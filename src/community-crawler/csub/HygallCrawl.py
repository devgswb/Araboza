...
class Crawler:
    ...
    def run(self, title, years, months, days):
        ...
        pool.close()
        sys.stdout.write(f"HyGall 크롤링 완료\r")
        sys.stdout.flush()

    def crawlPage(self, page):
        url = f'https://hygall.com/index.php?mid={self.title}&page={page}'
        time.sleep(0.02)
        res = req.get(url)
        res.encoding = None
        html = res.text
        ...
        for i, w in enumerate(soup.select('tr.docList.exBg0 > td.title > a.exJsHotTrackA')):
            if i > 0:
                w = w.get_text().strip()
                # w = w.split('\n')[0]
                w = w.replace('\n', '')
        ...
        for i, d in enumerate(soup.select('tr.docList.exBg0 > td.regdate')):
            if i > 0:
                d = d.get_text()
                d = d.replace('\n', '')
        ...
        for t in range(len(write)):
            ...
            if datetime.datetime.strptime(self.day, "%Y-%m-%d") < self.endTime:
                return
            name = 'HyGall'
            fpath = f'data/8/[{day[t].replace(".", "-")}]{name}.csv'
    ...
    def pageCalculation(self, date_Specified):  # 입력한 날짜의 글이 있는 페이지 까지 검색
        ...
        while stop == True:
            url = f'https://hygall.com/index.php?mid={self.title}&page={regen}'
            res = req.get(url)
            res.encoding = None
            html = res.text
