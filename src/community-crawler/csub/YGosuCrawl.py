...
class Crawler:
    ...
    def crawlPage(self, page):
        url = f'https://www.ygosu.com/community/issue/?page={page}'
        time.sleep(0.02)
        res = req.get(url)
        res.encoding = None
        html = res.text
        ...
        for i, w in enumerate(soup.select('tbody > tr:not(.notice) > td[class=tit] > a')):
            w = w.get_text()
            w = w.split('\n')[0]
            # w = w.replace('\n', '')
            w = w.replace(',', ' ')
        ...
        for i, d in enumerate(soup.select('tbody > tr:not(.notice) > td[class=date]')):
            d = d.get_text()
            d = d.replace('\n', '')
    ...
    def pageCalculation(self, date_Specified):  # 입력한 날짜의 글이 있는 페이지 까지 검색
        ...
        while stop == True:
            url = f'https://www.ygosu.com/community/issue/?page={regen}'
            res = req.get(url)
            res.encoding = None
            html = res.text
            soup = BeautifulSoup(html, 'html.parser', from_encoding='utf-8')
