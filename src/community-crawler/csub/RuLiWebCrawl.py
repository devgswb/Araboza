...
class Crawler:
    ...
    def run(self, years, months, days):
        ...
        pool.close()
        sys.stdout.write(f"Ruliweb 크롤링 완료\r")
        sys.stdout.flush()

    def crawlPage(self, page):
        url = f'http://bbs.ruliweb.com/community/board/300143?page={page}'
        time.sleep(0.02)
        res = req.get(url)
        res.encoding = None
        html = res.text
        ...
        write = []
        day = []
        for i, w in enumerate(soup.select('div[class=relative] > a[class=deco]')):
            if i > 1:
                w = w.get_text()
                w = w.split('\n')[0]
                # w = w.replace('\n', '')
        ...
        for i, d in enumerate(soup.select('tr[class=table_body] > td[class=time]')):
            if i > 1:
                d = d.get_text()
                d = d.replace('\n', '')
        ...
        for t in range(len(write)):
            ...
            if datetime.datetime.strptime(self.day, "%Y-%m-%d") < self.endTime:
                return
            name = 'Ruliweb'
            fpath = f'data/12/[{day[t].replace(".", "-")}]{name}.csv'
    ...
    def pageCalculation(self, date_Specified):  # 입력한 날짜의 글이 있는 페이지 까지 검색
        ...
        regen = 250  # 하루에 글이 평균적으로 작성되는 양(페이지 기준). 사이트마다 적당한 고정값을 줘야 검색이 빨라짐
        stop = True
        page_Target = True
        result = []
        ...
        while stop == True:
            url = f'http://bbs.ruliweb.com/community/board/300143?page={regen}'
            res = req.get(url)
            res.encoding = None
            html = res.text
            soup = BeautifulSoup(html, 'html.parser')
