...
class Crawler:
    ...
    def run(self, title, years, months, days):
        ...
        self.title = title
        self.noticeNum = 0
        self.url = f'http://bbs.ruliweb.com/community/board/{title}?page='
    ...
    def noticeCheck(self):
        ...
        for i, w in enumerate(soup.select('a.deco > strong')):
            self.noticeNum = self.noticeNum + 1

    def subjectCrawl(self, page, soup):
        write = []
        for i, w in enumerate(soup.select('div.relative > a.deco')):
            w = w.get_text()
            w = w.split('\n')[0]
            w = w.replace('\n', '')
            w = w.replace(',', ' ')
        ...
        try:
            ...
            for t in range(len(write)):
                ...
                if datetime.datetime.strptime(self.day, "%Y-%m-%d") < self.endTime:
                    return
                fpath = f'data/12/[{day[t].replace(".", "-")}]{self.name}.csv'
